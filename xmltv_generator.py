...
class VsetvGrabber:
    ...
    def get_channels_titles_dict(self):
        channels_titles = []
        i = 1
        for elem in self.g.doc.select('//td[@class="channeltitle"]').text_list():
            channels_titles.append({'display-name': [(elem, u'ru')], 'id': str(i)})
            i += 1
        return channels_titles
     
    def get_starttime(self):
        starttime = []
        for i in range(self.amount_channels):
            starttime.append(self.main_selector[i].select('./div[@class="time" or @class="onair" or @class="pasttime"]').text_list())
        return starttime
    ...
    def get_programmes_titles(self):
        title = []
        for i in range(self.amount_channels):
            title.append(self.main_selector[i].select('./div[@class="prname2" or @class="pastprname2"]').text_list())
        return title
