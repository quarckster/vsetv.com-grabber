...
class VsetvGrabber:
    ...
    def __init__(self):      
        ...
        self.amount_channels = range(len(self.g.doc.select('//div[@id="schedule_container"]')))
    ...
    def get_starttime(self):
        return [self.main_selector[i].select('./div[@class="time" or @class="onair" or @class="pasttime"]').text_list() for i in self.amount_channels]
    ...
    def get_programmes_titles(self):
        return [self.main_selector[i].select('./div[@class="prname2" or @class="pastprname2"]').text_list() for i in self.amount_channels]
